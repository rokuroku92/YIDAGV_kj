...
//                var len = myChart.data.labels.length;
    ...
    for(let i=0 ; i < data.length ; i++) {
        ...
        let openMinute = data[i].openMinute === 0 ? 1 : data[i].openMinute;
        myChart.data.datasets[0].data.push(((data[i].workingMinute/openMinute)*100).toString().substring(0,2));
        ...
        html += "<tr id=\""+data[i].month.toString()+data[i].day.toString()+"\"><th scope=\"row\">"+data[i].month + '/' + data[i].day+week+"</th><td>"+data[i].task+"</td><td>"+
                ((data[i].workingMinute/data[i].openMinute)*100).toFixed(1)+"%</td><td>"+Math.floor(Number(data[i].workingMinute)/60)+"</td><td>"+Math.floor(Number(data[i].openMinute)/60)+"</td>"+
    ...
    document.getElementById("task").value = String(Math.round(task_sum/x));
    ...
    document.getElementById("work").value = String(Math.round(work_sum/60/x))+"hr";
